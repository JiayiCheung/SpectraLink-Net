...
class EncoderBlock(nn.Module):
	...
	2. ImprovedFrequencyBranch: 提取3个频域token
	3. FreqGuidedFusion: 频域引导空域融合
	4. 输出 skip connection 特征

	结构：
	Input → SpatialBranch → (path1, path2, path3)
		  → FrequencyBranch → (token0, token1, token2)
	...
	def __init__(
		...
		# 空域分支：输出3个path
		# 通道分配：1/2, 1/4, 1/4
		# path1 (3×3×3): out_channels // 2
		# path2 (5×5×5): out_channels // 4
		# path3 (7×7×7): out_channels // 4
		self.spatial_branch = EnhancedSpatialBranch(
			in_channels=in_channels,
			out_channels=out_channels  # 总通道数，内部按比例分配
		)
		...
		if downsample:
			self.downsample = DownsampleBlock(
				...
				out_channels=out_channels * 2  # 下采样时通道翻倍
			)
		else:
...
class Encoder(nn.Module):
	...
	def __init__(
		...
		# Bottleneck: 最后一层下采样输出再做一次卷积
		# 输入: [B, 256, 8, 16, 16]（假设3层，base=32）
